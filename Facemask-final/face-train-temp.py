		...
		if (label=='No Mask' and image_count==10):
			...
			for encodeFace,faceLoc in zip(encodesCurFrame,facesCurFrame):
				...
				if matches[matchIndex]:
					...
					#Converting the captured image to binary form to insert into database
					img_for_db = converted_image() 


					if not ((date,rec_name) in visited):

						visited.append((date,rec_name)) 
						#Interating with the database


						#selecting all the student details whose face has been detected as 'No Mask'
						...
						#Inserting captured image and few details into the database
						Table_Query = "INSERT INTO fine_details (sno, date, roll_no, name, fine,captured_image) VALUES (%s,%s,%s,%s,%s,%s) "
						val=(image_serial,date,roll_no,rec_name,"50",img_for_db)
						...
						print(record)
						#Sending an Email alert to the Student
